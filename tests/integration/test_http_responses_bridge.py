...
def _make_dummy_bridge_session(
    ...
    *,
    affinity: proxy_module._AffinityPolicy | None = None,
    account_id: str | None = None,
    codex_session: bool | None = None,
) -> SimpleNamespace:
    async def _close() -> None:
        return None

    effective_affinity = affinity or proxy_module._AffinityPolicy()
    return SimpleNamespace(
        key=session_key,
        closed=False,
        headers={},
        affinity=effective_affinity,
        account=SimpleNamespace(id=account_id or f"acct_{session_key.affinity_key}", status=AccountStatus.ACTIVE),
        ...
        codex_session=(
            effective_affinity.kind == proxy_module.StickySessionKind.CODEX_SESSION
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_threshold_rotation_rebinds_idle_codex_session(app_instance, monkeypatch):
    service = get_proxy_service_for_app(app_instance)
    service._http_bridge_sessions.clear()
    service._http_bridge_inflight_sessions.clear()
    service._http_bridge_turn_state_index.clear()
    ...
    _install_bridge_settings_with_limits(
        ...
        enabled=True,
        codex_idle_ttl_seconds=600.0,
        routing_strategy="threshold_rotation",
    )

    key = proxy_module._HTTPBridgeSessionKey("session_header", "codex-session-bridge-1", None)
    affinity = proxy_module._AffinityPolicy(
        key="codex-session-bridge-1",
        kind=proxy_module.StickySessionKind.CODEX_SESSION,
    )
    existing_session = cast(
        ...
        _make_dummy_bridge_session(
            key,
            affinity=affinity,
            account_id="acc_threshold_old",
            codex_session=True,
        ),
    )
    replacement_session = cast(
        proxy_module._HTTPBridgeSession,
        _make_dummy_bridge_session(
            key,
            affinity=affinity,
            account_id="acc_threshold_new",
            codex_session=True,
        ),
    )
    ...
    async def fake_create_http_bridge_session(
        ...
        *,
        ...
        del self, key, headers, affinity, request_model, idle_ttl_seconds
        return replacement_session

    monkeypatch.setattr(proxy_module.ProxyService, "_create_http_bridge_session", fake_create_http_bridge_session)
    select_account = AsyncMock(
        return_value=AccountSelection(
            account=SimpleNamespace(id="acc_threshold_new", status=AccountStatus.ACTIVE),
            error_message=None,
            error_code=None,
        )
    )
    ...
    selected = await service._get_or_create_http_bridge_session(
        key,
        headers={"session_id": "codex-session-bridge-1"},
        affinity=affinity,
        api_key=None,
        request_model="gpt-5.4",
        idle_ttl_seconds=120.0,
    ...
    assert selected is replacement_session
    ...
    select_account.assert_awaited_once_with(
        sticky_key="codex-session-bridge-1",
        sticky_kind=proxy_module.StickySessionKind.CODEX_SESSION,
...
@pytest.mark.asyncio
async def test_v1_responses_http_bridge_threshold_rotation_keeps_busy_codex_session(app_instance, monkeypatch):
    service = get_proxy_service_for_app(app_instance)
    service._http_bridge_sessions.clear()
    service._http_bridge_inflight_sessions.clear()
    service._http_bridge_turn_state_index.clear()
    ...
    key = proxy_module._HTTPBridgeSessionKey("session_header", "codex-session-bridge-busy", None)
    affinity = proxy_module._AffinityPolicy(
        key="codex-session-bridge-busy",
        kind=proxy_module.StickySessionKind.CODEX_SESSION,
    )
    existing_session = cast(
        proxy_module._HTTPBridgeSession,
        _make_dummy_bridge_session(
            key,
            affinity=affinity,
            account_id="acc_threshold_busy",
            codex_session=True,
        ),
    )
    existing_session.pending_requests.append(SimpleNamespace(request_id="req-busy"))
    service._http_bridge_sessions[key] = existing_session

    create_session = AsyncMock()
    monkeypatch.setattr(proxy_module.ProxyService, "_create_http_bridge_session", create_session)
    select_account = AsyncMock()
    monkeypatch.setattr(service._load_balancer, "select_account", select_account)

    selected = await service._get_or_create_http_bridge_session(
        key,
        headers={"session_id": "codex-session-bridge-busy"},
        affinity=affinity,
        api_key=None,
        request_model="gpt-5.4",
        idle_ttl_seconds=120.0,
    ...
    assert selected is existing_session
...
@pytest.mark.asyncio
async def test_use_only_account_closes_http_bridge_sessions_for_other_accounts(async_client, app_instance):
    selected_account_id = await _import_account(
        async_client,
        "acc_http_bridge_use_only_selected",
        "http-bridge-use-only-selected@example.com",
    )
    previous_account_id = await _import_account(
        async_client,
        "acc_http_bridge_use_only_previous",
        "http-bridge-use-only-previous@example.com",
    )
    service = get_proxy_service_for_app(app_instance)

    keep_key = proxy_module._HTTPBridgeSessionKey("prompt_cache", "exclusive-keep", None)
    keep_session = cast(proxy_module._HTTPBridgeSession, _make_dummy_bridge_session(keep_key))
    keep_session.account = SimpleNamespace(id=selected_account_id, status=AccountStatus.ACTIVE)

    close_key = proxy_module._HTTPBridgeSessionKey("prompt_cache", "exclusive-close", None)
    close_session = cast(proxy_module._HTTPBridgeSession, _make_dummy_bridge_session(close_key))
    close_session.account = SimpleNamespace(id=previous_account_id, status=AccountStatus.ACTIVE)
    close_session.downstream_turn_state_aliases.add("http_turn_exclusive_close")

    service._http_bridge_sessions[keep_key] = keep_session
    service._http_bridge_sessions[close_key] = close_session
    service._http_bridge_turn_state_index[("http_turn_exclusive_close", None)] = close_key

    response = await async_client.post(f"/api/accounts/{selected_account_id}/use-only")
    assert response.status_code == 200
    assert response.json()["status"] == "exclusive_activated"

    assert keep_key in service._http_bridge_sessions
    assert service._http_bridge_sessions[keep_key] is keep_session
    assert close_key not in service._http_bridge_sessions
    assert ("http_turn_exclusive_close", None) not in service._http_bridge_turn_state_index
    assert close_session.closed is True
