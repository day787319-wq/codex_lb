...
def _install_proxy_settings_cache(
    ...
    *,
    ...
    routing_strategy: str = "usage_weighted",
    threshold_rotation_account_order: list[str] | None = None,
    threshold_rotation_step_percent: int = 10,
    threshold_rotation_min_remaining_percent: int = 10,
) -> None:
    settings = SimpleNamespace(
        ...
        threshold_rotation_account_order=threshold_rotation_account_order or [],
        threshold_rotation_step_percent=threshold_rotation_step_percent,
        threshold_rotation_min_remaining_percent=threshold_rotation_min_remaining_percent,
...
@pytest.mark.asyncio
async def test_proxy_codex_session_id_threshold_rotation_rebinds_after_step_drop(async_client, monkeypatch):
    await _set_routing_settings(
        async_client,
    ...
    _install_proxy_settings_cache(
        ...
        sticky_threads_enabled=False,
        routing_strategy="threshold_rotation",
        threshold_rotation_account_order=[
            "threshold_sid_a@example.com",
            "threshold_sid_b@example.com",
        ],
    )
    upstream_acc_a = "acc_threshold_sid_a"
    upstream_acc_b = "acc_threshold_sid_b"
    acc_a_id = await _import_account(async_client, "acc_threshold_sid_a", "threshold_sid_a@example.com")
    acc_b_id = await _import_account(async_client, "acc_threshold_sid_b", "threshold_sid_b@example.com")
    ...
    async def fake_stream(payload, headers, access_token, account_id, base_url=None, raise_for_status=False, **_kwargs):
        stream_seen.append(account_id)
        yield 'data: {"type":"response.completed","response":{"id":"resp_threshold_rebind"}}\n\n'

    monkeypatch.setattr(proxy_module, "core_stream_responses", fake_stream)

    headers = {"session_id": "codex-threshold-rebind-123"}
    stream_payload = {
        "model": "gpt-5.1",
        "instructions": "hi",
        "input": [],
    ...
    response = await async_client.post("/backend-api/codex/responses", json=stream_payload, headers=headers)
    assert response.status_code == 200
    assert stream_seen == [upstream_acc_a, upstream_acc_b]

    async with SessionLocal() as session:
        sticky_row = (
            await session.execute(
                text("SELECT account_id, kind FROM sticky_sessions WHERE key = :key"),
                {"key": "codex-threshold-rebind-123"},
            )
        ).fetchone()
        assert sticky_row is not None
        assert sticky_row[0] == acc_b_id
        assert sticky_row[1] == "codex_session"

    response = await async_client.post("/backend-api/codex/responses", json=stream_payload, headers=headers)
    assert response.status_code == 200
    assert stream_seen == [upstream_acc_a, upstream_acc_b, upstream_acc_b]
