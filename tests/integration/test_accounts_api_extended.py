...
@pytest.mark.asyncio
async def test_use_only_account_activates_target_and_pauses_other_non_deactivated_accounts(async_client, db_setup):
    async with SessionLocal() as session:
        accounts_repo = AccountsRepository(session)

        await accounts_repo.upsert(_make_account("acc_use_only_target", "target@example.com", status=AccountStatus.PAUSED))
        await accounts_repo.upsert(_make_account("acc_use_only_other", "other@example.com"))
        await accounts_repo.upsert(
            _make_account(
                "acc_use_only_deactivated",
                "deactivated@example.com",
                status=AccountStatus.DEACTIVATED,
            )
        )

    response = await async_client.post("/api/accounts/acc_use_only_target/use-only")
    assert response.status_code == 200
    ...
    assert accounts["acc_use_only_target"]["status"] == "active"
    assert accounts["acc_use_only_other"]["status"] == "paused"
    assert accounts["acc_use_only_deactivated"]["status"] == "deactivated"


@pytest.mark.asyncio
async def test_use_only_account_clears_sticky_sessions_for_other_accounts(async_client, db_setup):
    async with SessionLocal() as session:
        accounts_repo = AccountsRepository(session)
        ...
        await accounts_repo.upsert(_make_account("acc_use_only_target", "target@example.com", status=AccountStatus.PAUSED))
        await accounts_repo.upsert(_make_account("acc_use_only_other", "other@example.com"))
        await accounts_repo.upsert(_make_account("acc_use_only_third", "third@example.com"))
        ...
        await sticky_repo.upsert(
            "exclusive-other-cache",
            "acc_use_only_other",
            kind=StickySessionKind.PROMPT_CACHE,
        )
        await sticky_repo.upsert(
            "exclusive-third-thread",
            "acc_use_only_third",
    ...
    async with SessionLocal() as session:
        rows = (
            await session.execute(
                select(StickySession).order_by(StickySession.key.asc(), StickySession.kind.asc())
            )
        ).scalars().all()

    assert [(row.key, row.kind, row.account_id) for row in rows] == [
        ("exclusive-target-session", StickySessionKind.CODEX_SESSION, "acc_use_only_target")
    ]
