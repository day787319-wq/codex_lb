...
def _threshold_rotation_order_map() -> dict[str, int]:
    order = get_settings().threshold_rotation_account_order
    return {identifier: index for index, identifier in enumerate(order)}


def _threshold_rotation_rank_for_account(
    account: Account,
    ...
    if not order_map:
        return None
    candidates = [
        account.id.strip().lower(),
        account.email.strip().lower(),
    ]
    if account.chatgpt_account_id:
        candidates.append(account.chatgpt_account_id.strip().lower())
    for candidate in candidates:
        rank = order_map.get(candidate)
        if rank is not None:
            return rank
    return None
...
class LoadBalancer:
        ...
        if sticky_key is None:
            while True:
                async with self._runtime_lock:
                    ...
                    threshold_rotation_order = _threshold_rotation_order_map()
                    states, account_map = _build_states(
                        accounts=selection_inputs.accounts,
                        latest_primary=selection_inputs.latest_primary,
                        latest_secondary=selection_inputs.latest_secondary,
                        runtime=self._runtime,
                        threshold_rotation_order=threshold_rotation_order,
                    )
                    runtime_versions = {
                ...
                result = select_account(
                    ...
                    threshold_rotation_step_percent=get_settings().threshold_rotation_step_percent,
                    threshold_rotation_min_remaining_percent=get_settings().threshold_rotation_min_remaining_percent,
                )
        ...
        *,
        ...
        if existing:
            ...
            if pinned is not None:
                if sticky_kind == StickySessionKind.CODEX_SESSION and routing_strategy == "threshold_rotation":
                    threshold_result = select_account(
                        states,
                        prefer_earlier_reset=prefer_earlier_reset_accounts,
                        routing_strategy=routing_strategy,
                        threshold_rotation_step_percent=get_settings().threshold_rotation_step_percent,
                    ...
                    if threshold_result.account is None:
                        return threshold_result
                    if threshold_result.account.account_id != pinned.account_id:
                        if threshold_result.account.account_id in account_map:
                            await sticky_repo.upsert(
                                sticky_key,
                                threshold_result.account.account_id,
                                kind=sticky_kind,
                            )
                        return threshold_result
    ...
    def _state_for(self, account: Account) -> AccountState:
        ...
        return AccountState(
            ...
            threshold_rotation_rank=_threshold_rotation_rank_for_account(account, _threshold_rotation_order_map()),
...
def _build_states(
    *,
    ...
    threshold_rotation_order: dict[str, int],
...
def _state_from_account(
    *,
    ...
    return AccountState(
        ...
        last_selected_at=runtime.last_selected_at,
        error_count=runtime.error_count,
        threshold_rotation_rank=_threshold_rotation_rank_for_account(account, threshold_rotation_order),
        deactivation_reason=account.deactivation_reason,
    )
