...
RoutingStrategy = Literal["usage_weighted", "round_robin", "threshold_rotation"]
...
class AccountState:
    ...
    error_count: int = 0
    threshold_rotation_rank: int | None = None
...
def select_account(
    ...
    *,
    ...
    elif routing_strategy == "threshold_rotation":
        selected = _threshold_rotation_select(
            available,
            step_percent=threshold_rotation_step_percent,
            min_remaining_percent=threshold_rotation_min_remaining_percent,
        )
        if selected is None:
            return SelectionResult(None, "Threshold rotation waiting for primary quota reset")
    else:
        selected = min(available, key=_reset_first_sort_key if prefer_earlier_reset else _usage_sort_key)
    return SelectionResult(selected, None)


def _threshold_rotation_select(
    available: list[AccountState],
    *,
    step_percent: int,
    ...
    ordered_states = sorted(
        (state for state in available if state.threshold_rotation_rank is not None),
        key=lambda state: (state.threshold_rotation_rank or 0, state.account_id),
    )
    if not ordered_states:
        return min(available, key=lambda state: (
            state.used_percent if state.used_percent is not None else 0.0,
            state.last_selected_at or 0.0,
            state.account_id,
        ))

    threshold = 100 - step_percent
    while threshold >= min_remaining_percent:
        for state in ordered_states:
            if _primary_remaining_percent(state) > threshold:
                return state
        threshold -= step_percent
    return None


def _primary_remaining_percent(state: AccountState) -> float:
    used_percent = state.used_percent if state.used_percent is not None else 0.0
    clamped_used_percent = max(0.0, min(100.0, used_percent))
    return max(0.0, 100.0 - clamped_used_percent)
