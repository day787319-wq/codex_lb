...
class Settings(BaseSettings):
    ...
    threshold_rotation_account_order: Annotated[list[str], NoDecode] = Field(default_factory=list)
    threshold_rotation_step_percent: int = Field(default=10, gt=0, lt=100)
    threshold_rotation_min_remaining_percent: int = Field(default=10, ge=0, lt=100)
    ...
    @field_validator("threshold_rotation_account_order", mode="before")
    @classmethod
    def _normalize_threshold_rotation_account_order(cls, value: object) -> list[str]:
        if value is None:
            return []
        if isinstance(value, str):
        ...
        if isinstance(value, list):
            ...
            return normalized
        raise TypeError("threshold_rotation_account_order must be a list or comma-separated string")
    ...
    def _validate_http_bridge_instance_configuration(self) -> "Settings":
        ...
        if self.threshold_rotation_min_remaining_percent >= 100 - self.threshold_rotation_step_percent:
            raise ValueError(
                "threshold_rotation_min_remaining_percent must be below the first threshold band "
                "(100 - threshold_rotation_step_percent)"
            )
        return self
