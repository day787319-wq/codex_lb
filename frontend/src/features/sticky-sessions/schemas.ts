...
export const CurrentCodexSessionEntrySchema = StickySessionEntrySchema.extend({
  turnState: z.string().min(1).nullable(),
  liveBridgeConnected: z.boolean().default(false),
});

export const CurrentCodexSessionUsageResponseSchema = z.object({
  current: CurrentCodexSessionEntrySchema.nullable(),
  otherSessionsUsingAccount: z.array(StickySessionEntrySchema).default([]),
});
...
export const StickySessionCreateRequestSchema = z.object({
  key: z.string().trim().min(1),
  kind: StickySessionKindSchema,
  accountId: z.string().trim().min(1),
});

export const StickySessionCreateResponseSchema = z.object({
  status: z.literal("saved"),
  entry: StickySessionEntrySchema,
});
...
export type CurrentCodexSessionEntry = z.infer<typeof CurrentCodexSessionEntrySchema>;
export type CurrentCodexSessionUsageResponse = z.infer<typeof CurrentCodexSessionUsageResponseSchema>;
...
export type StickySessionCreateRequest = z.infer<typeof StickySessionCreateRequestSchema>;
export type StickySessionCreateResponse = z.infer<typeof StickySessionCreateResponseSchema>;
