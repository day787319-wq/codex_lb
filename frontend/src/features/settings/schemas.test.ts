...
describe("DashboardSettingsSchema", () => {
  ...
  it("accepts threshold rotation as a routing mode", () => {
    const parsed = DashboardSettingsSchema.parse({
      stickyThreadsEnabled: false,
      upstreamStreamTransport: "default",
      preferEarlierResetAccounts: false,
      routingStrategy: "threshold_rotation",
      openaiCacheAffinityMaxAgeSeconds: 300,
    ...
    expect(parsed.routingStrategy).toBe("threshold_rotation");
  });
});
...
describe("SettingsUpdateRequestSchema", () => {
  ...
  it("accepts threshold rotation updates", () => {
    const parsed = SettingsUpdateRequestSchema.parse({
      stickyThreadsEnabled: false,
      preferEarlierResetAccounts: false,
      routingStrategy: "threshold_rotation",
