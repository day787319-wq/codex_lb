...
export function RoutingSettings({ settings, busy, onSave }: RoutingSettingsProps) {
  ...
  const routingStrategyDescription = settings.routingStrategy === "threshold_rotation"
    ? "Rotate by primary quota bands. Configure the ordered accounts in env settings."
    : "Choose usage-based balancing, strict round robin, or threshold rotation.";
        ...
        <div className="divide-y rounded-lg border">
          ...
          <div className="flex items-center justify-between gap-4 p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">{routingStrategyDescription}</p>
            </div>
            <Select
              value={settings.routingStrategy}
              onValueChange={(value) =>
                save({ routingStrategy: value as "usage_weighted" | "round_robin" | "threshold_rotation" })
              }
            >
              <SelectTrigger className="h-8 w-44 text-xs" disabled={busy}>
                <SelectValue />
              ...
              <SelectContent align="end">
                ...
                <SelectItem value="threshold_rotation">Threshold rotation</SelectItem>
              </SelectContent>
            </Select>
          </div>
