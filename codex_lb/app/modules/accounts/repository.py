...
class AccountsRepository:
    ...
    async def activate_exclusively(self, account_id: str) -> bool:
        target = await self._session.get(Account, account_id)
        if target is None:
            return False
        ...
        await self._session.execute(
            ...
            .values(status=AccountStatus.PAUSED, deactivation_reason=None, reset_at=None)
        )
        await self._session.execute(
            update(Account)
            .where(Account.id == account_id)
            .values(status=AccountStatus.ACTIVE, deactivation_reason=None, reset_at=None)
        )
        await self._session.commit()
        return True
