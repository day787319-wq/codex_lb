...
class StickySessionsRepository:
    ...
    async def get_entry_record(self, key: str, *, kind: StickySessionKind) -> StickySessionListEntryRecord | None:
        if not key:
            return None
        statement = (
        ...
        if row is None:
            return None
        sticky_session, display_name = row
        return StickySessionListEntryRecord(sticky_session=sticky_session, display_name=display_name)
    ...
    async def delete_for_other_accounts(self, account_id: str) -> int:
        statement = delete(StickySession).where(StickySession.account_id != account_id)
        result = await self._session.execute(statement.returning(StickySession.key))
