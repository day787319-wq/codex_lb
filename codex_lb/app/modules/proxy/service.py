...
@dataclass(frozen=True, slots=True)
class LiveCodexBridgeSessionSnapshot:
    account_id: str
    affinity_key: str
    affinity_kind: str
    downstream_turn_state: str | None
    upstream_turn_state: str | None
    turn_state_aliases: tuple[str, ...]
...
class ProxyService:
    ...
    async def invalidate_affinity_for_exclusive_account(self, account_id: str) -> None:
        async with self._repo_factory() as repos:
            deleted_sticky_sessions = await repos.sticky_sessions.delete_for_other_accounts(account_id)

        sessions_to_close: list[_HTTPBridgeSession] = []
        async with self._http_bridge_lock:
            for key, session in list(self._http_bridge_sessions.items()):
        ...
        logger.info(
            "Invalidated affinity for exclusive account_id=%s deleted_sticky_sessions=%s closed_http_bridge_sessions=%s",
            account_id,
            deleted_sticky_sessions,
            len(sessions_to_close),
        )

    async def list_live_codex_bridge_sessions(self) -> list[LiveCodexBridgeSessionSnapshot]:
        snapshots: list[LiveCodexBridgeSessionSnapshot] = []
        async with self._http_bridge_lock:
            for session in self._http_bridge_sessions.values():
                if session.closed or not session.codex_session or session.account.status != AccountStatus.ACTIVE:
                    continue
                snapshots.append(
                    LiveCodexBridgeSessionSnapshot(
                        ...
                        turn_state_aliases=tuple(sorted(session.downstream_turn_state_aliases)),
                    )
                )
        return snapshots
        ...
        while True:
            ...
            async with self._http_bridge_lock:
                ...
                if existing is not None and not existing.closed and existing.account.status == AccountStatus.ACTIVE:
                    if not await self._should_reuse_http_bridge_session(
                        existing,
    ...
    async def _should_reuse_http_bridge_session(
        self,
        ...
        *,
        ...
        if session.closed or session.account.status != AccountStatus.ACTIVE:
            return False
        if session.affinity.kind != StickySessionKind.CODEX_SESSION:
            return True
        if _routing_strategy(settings) != "threshold_rotation":
            return True
        if session.affinity.key is None:
            return True
        ...
        if selection.account is None:
            _log_http_bridge_event(
                "threshold_rotation_rebind",
                session.key,
                account_id=session.account.id,
                model=request_model,
                detail="selected_account_id=none",
            )
            return False
        if selection.account.id != session.account.id:
            _log_http_bridge_event(
                "threshold_rotation_rebind",
                session.key,
                account_id=session.account.id,
                model=request_model,
                detail=f"selected_account_id={selection.account.id}",
            )
            return False
        return True
...
def _routing_strategy(settings: DashboardSettings) -> RoutingStrategy:
    ...
    if value == "round_robin":
        return "round_robin"
    if value == "threshold_rotation":
        return "threshold_rotation"
