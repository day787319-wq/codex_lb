...
const SettingsPayloadSchema = z.object({
  ...
  routingStrategy: z.enum(["usage_weighted", "round_robin", "threshold_rotation"]).optional(),
...
const StickySessionCreatePayloadSchema = z.object({
  key: z.string().min(1),
...
export const handlers = [
  ...
  http.post("/api/accounts/:accountId/use-only", ({ params }) => {
    const accountId = String(params.accountId);
    const account = findAccount(accountId);
    if (!account) {
      return HttpResponse.json(
    ...
    return HttpResponse.json({ status: "exclusive_activated" });
  }),
  ...
  http.get("/api/sticky-sessions", ({ request }) => {
    ...
    const filteredEntries = staleOnly
      ? kindFilteredEntries.filter((entry) => entry.kind === "prompt_cache" && entry.isStale)
      : kindFilteredEntries;
  ...
  http.get("/api/sticky-sessions/current-codex-session", () => {
    const codexSessions = state.stickySessions
      .filter((entry) => entry.kind === "codex_session")
      .sort((left, right) => new Date(right.updatedAt).getTime() - new Date(left.updatedAt).getTime());
    const current = codexSessions[0] ?? null;
    const otherSessionsUsingAccount = current
      ? codexSessions.filter((entry) => entry.accountId === current.accountId && entry.key !== current.key)
      : [];
    return HttpResponse.json({
      current: current
        ? {
            ...current,
            turnState:
              current.key.startsWith("turn_") || current.key.startsWith("http_turn_")
                ? current.key
                : null,
            liveBridgeConnected:
              current.key.startsWith("turn_") || current.key.startsWith("http_turn_"),
  ...
  http.post("/api/sticky-sessions", async ({ request }) => {
    const payload = await parseJsonBody(request, StickySessionCreatePayloadSchema);
    if (!payload) {
      return HttpResponse.json(
        { error: { code: "validation_error", message: "Invalid sticky session payload" } },
        { status: 422 },
      );
    }
    const account = state.accounts.find((entry) => entry.accountId === payload.accountId);
    if (!account) {
      return HttpResponse.json(
        { error: { code: "account_not_found", message: "Account not found" } },
        { status: 404 },
    ...
    const existing = state.stickySessions.findIndex((entry) => entry.key === payload.key && entry.kind === payload.kind);
    const entry = {
      key: payload.key,
      accountId: payload.accountId,
      displayName: account.displayName,
      kind: payload.kind,
      createdAt: existing >= 0 ? state.stickySessions[existing]!.createdAt : now,
      updatedAt: now,
      expiresAt: payload.kind === "prompt_cache" ? now : null,
      isStale: false,
    };
    if (existing >= 0) {
      state.stickySessions[existing] = entry;
    } else {
      state.stickySessions.unshift(entry);
    }
    return HttpResponse.json({ status: "saved", entry });
  }),
