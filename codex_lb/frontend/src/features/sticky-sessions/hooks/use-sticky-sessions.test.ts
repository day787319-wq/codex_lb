...
describe("useStickySessions", () => {
  it("loads sticky sessions and invalidates on delete/purge", async () => {
    ...
    server.use(
      ...
      http.post("/api/sticky-sessions", async ({ request }) => {
        const payload = (await request.json()) as { key: string; kind: string; accountId: string };
        const created = {
          key: payload.key,
          accountId: payload.accountId,
          displayName: "sticky-a@example.com",
          kind: payload.kind as "codex_session" | "sticky_thread" | "prompt_cache",
          createdAt: "2026-03-10T12:00:00Z",
          updatedAt: "2026-03-10T12:05:00Z",
          expiresAt: null,
          isStale: false,
        ...
        return HttpResponse.json({ status: "saved", entry: created });
      }),
    ...
    await waitFor(() => {
      expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ["sticky-sessions"] });
    });
  ...
  it("uses fallback toast messages when sticky-session mutations fail", async () => {
    ...
    const createSpy = vi
      .spyOn(stickySessionsApi, "createStickySession")
    ...
    expect(toastSpy).toHaveBeenCalledWith("Failed to save sticky session");
