...
describe("StickySessionsSection", () => {
  ...
  it("renders rows and supports purge and remove actions", async () => {
    ...
    expect(screen.getAllByText("Prompt cache")).not.toHaveLength(0);
  ...
  it("submits a manual sticky-session mapping", async () => {
    const user = userEvent.setup();
    ...
    useStickySessionsMock.mockReturnValue(buildHookState({ createMutation }));

    render(<StickySessionsSection />);

    await user.type(screen.getByLabelText("Key"), "manual-session-1");
    await user.click(screen.getByRole("button", { name: "Save mapping" }));

    await waitFor(() => {
      expect(createMutation.mutateAsync).toHaveBeenCalledWith({
        key: "manual-session-1",
        kind: "codex_session",
        accountId: "acc_primary",
      });
    });
