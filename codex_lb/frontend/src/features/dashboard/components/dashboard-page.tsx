...
import { useCurrentCodexSession } from "@/features/dashboard/hooks/use-current-codex-session";
...
export function DashboardPage() {
  ...
  const currentCodexSessionQuery = useCurrentCodexSession();
  const { filters, logsQuery, optionsQuery, updateFilters } = useRequestLogs();
  const { resumeMutation, useOnlyMutation } = useAccountMutations();
  ...
  const handleAccountAction = useCallback(
    (account: AccountSummary, action: string) => {
      switch (action) {
        ...
        case "resume":
          ...
          break;
        case "use_only":
          void useOnlyMutation.mutateAsync(account.accountId);
          break;
        case "reauth":
          navigate(`/accounts?selected=${account.accountId}`);
          break;
    ...
    [navigate, resumeMutation, useOnlyMutation],
  );
        ...
        <>
          ...
          <section className="space-y-4">
            ...
            <AccountCards
              ...
              currentRoutedAccountId={currentCodexSessionQuery.data?.accountId ?? null}
