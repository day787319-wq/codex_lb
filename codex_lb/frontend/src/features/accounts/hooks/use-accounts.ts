...
export function useAccountMutations() {
  ...
  const useOnlyMutation = useMutation({
    mutationFn: useOnlyAccount,
    onSuccess: () => {
      toast.success("Now using this account only");
      invalidateAccountRelatedQueries(queryClient);
    },
    onError: (error: Error) => {
      toast.error(error.message || "Exclusive account switch failed");
    },
  });
  ...
  return { importMutation, pauseMutation, resumeMutation, useOnlyMutation, deleteMutation };
}
