...
@pytest.mark.asyncio
async def test_select_account_threshold_rotation_rebinds_codex_session_to_higher_band(monkeypatch) -> None:
    account_a = _make_account("acc-threshold-session-a", "threshold-session-a@example.com")
    account_b = _make_account("acc-threshold-session-b", "threshold-session-b@example.com")
    now = utcnow()
    now_epoch = int(now.replace(tzinfo=timezone.utc).timestamp())
    ...
    monkeypatch.setattr(
        ...
        "get_settings",
        lambda: SimpleNamespace(
            threshold_rotation_account_order=[account_a.email, account_b.email],
            threshold_rotation_step_percent=10,
            threshold_rotation_min_remaining_percent=10,
        ),
    )
    monkeypatch.setattr(sticky_repo, "get_account_id", AsyncMock(return_value=account_a.id))
    upsert = AsyncMock(return_value=StickySession(key="codex-session-1", account_id=account_b.id, kind=StickySessionKind.CODEX_SESSION))
    ...
    upsert.assert_awaited_once_with("codex-session-1", account_b.id, kind=StickySessionKind.CODEX_SESSION)
