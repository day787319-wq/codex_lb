...
def test_select_account_threshold_rotation_prefers_first_ranked_account_in_highest_band():
    states = [
        AccountState("a", AccountStatus.ACTIVE, used_percent=0.0, threshold_rotation_rank=0),
        AccountState("b", AccountStatus.ACTIVE, used_percent=0.0, threshold_rotation_rank=1),
        AccountState("c", AccountStatus.ACTIVE, used_percent=2.0, threshold_rotation_rank=2),
    ]
    result = select_account(states, routing_strategy="threshold_rotation")
    assert result.account is not None
    assert result.account.account_id == "a"


def test_select_account_threshold_rotation_advances_within_band():
    states = [
        AccountState("a", AccountStatus.ACTIVE, used_percent=10.0, threshold_rotation_rank=0),
        AccountState("b", AccountStatus.ACTIVE, used_percent=0.0, threshold_rotation_rank=1),
        AccountState("c", AccountStatus.ACTIVE, used_percent=1.0, threshold_rotation_rank=2),
    ]
    result = select_account(states, routing_strategy="threshold_rotation")
    assert result.account is not None
    assert result.account.account_id == "b"
...
def test_select_account_threshold_rotation_drops_to_next_band_and_restarts_order():
    states = [
        AccountState("a", AccountStatus.ACTIVE, used_percent=10.0, threshold_rotation_rank=0),
        AccountState("b", AccountStatus.ACTIVE, used_percent=10.0, threshold_rotation_rank=1),
        AccountState("c", AccountStatus.ACTIVE, used_percent=15.0, threshold_rotation_rank=2),
    ]
    result = select_account(states, routing_strategy="threshold_rotation")
    assert result.account is not None
    assert result.account.account_id == "a"
...
def test_select_account_threshold_rotation_skips_paused_accounts():
    states = [
        AccountState("a", AccountStatus.PAUSED, used_percent=0.0, threshold_rotation_rank=0),
        AccountState("b", AccountStatus.ACTIVE, used_percent=0.0, threshold_rotation_rank=1),
        AccountState("c", AccountStatus.ACTIVE, used_percent=2.0, threshold_rotation_rank=2),
    ]
    result = select_account(states, routing_strategy="threshold_rotation")
...
def test_select_account_threshold_rotation_waits_after_min_band_is_exhausted():
    states = [
        AccountState("a", AccountStatus.ACTIVE, used_percent=90.0, threshold_rotation_rank=0),
        AccountState("b", AccountStatus.ACTIVE, used_percent=95.0, threshold_rotation_rank=1),
    ]
    result = select_account(states, routing_strategy="threshold_rotation")
    assert result.account is None
    assert result.error_message == "Threshold rotation waiting for primary quota reset"


def test_select_account_threshold_rotation_falls_back_when_no_ranked_accounts():
    states = [
        AccountState("a", AccountStatus.ACTIVE, used_percent=20.0),
